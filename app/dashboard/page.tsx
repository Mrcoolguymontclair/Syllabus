...
function Toast({ message, onUndo, visible }: { message: string; onUndo: () => void; visible: boolean }) {
  return (
    ...
    >
      <div className="bg-gray-900 text-white text-sm px-4 py-3 rounded-xl shadow-lg flex items-center gap-3">
        <span>{message}</span>
        <button onClick={onUndo} className="text-blue-300 hover:text-blue-200 font-medium">Undo</button>
      </div>
    </div>
  )
}
...
function EditableTitle({ value, onSave, muted }: { value: string; onSave: (v: string) => void; muted: boolean }) {
  ...
  if (!editing) {
    return (
      <button
        ...
        className={`text-left font-medium max-w-xs truncate cursor-text hover:underline underline-offset-2 decoration-dashed decoration-gray-300 ${muted ? 'line-through text-gray-400' : 'text-gray-900'}`}
      >
        {value}
      </button>
    )
  ...
  return (
    <input ref={inputRef} value={draft}
      ...
      className="w-full border border-blue-300 rounded px-1.5 py-0.5 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500"
    />
  )
}
...
function EditableClassName({ value, onSave }: { value: string; onSave: (v: string) => void }) {
  ...
  if (!editing) {
    return (
      <button onClick={() => setEditing(true)} className="text-left hover:underline underline-offset-2 decoration-dashed decoration-gray-300 cursor-text">
        {value}
      </button>
    )
  }
  return (
    <input ref={inputRef} value={draft}
      onChange={(e) => setDraft(e.target.value)} onBlur={commit}
      onKeyDown={(e) => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') { setDraft(value); setEditing(false) } }}
      className="w-full border border-blue-300 rounded px-1.5 py-0.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
    />
  )
}
...
function EditableDueDate({ value, onSave, muted }: { value: string; onSave: (v: string) => void; muted: boolean }) {
  ...
  if (!editing) {
    return (
      <button
        onClick={() => setEditing(true)}
        className={`whitespace-nowrap cursor-text hover:underline underline-offset-2 decoration-dashed decoration-gray-300 ${muted ? 'line-through text-gray-400' : dueDateColor(value)}`}
      >
        {formatDueDate(value)}
      </button>
    )
  ...
  return (
    <input
      ...
      className="border border-blue-300 rounded px-1.5 py-0.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
    />
  )
}
    ...
    <main className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <header className="bg-white border-b border-gray-200 px-4 sm:px-8 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">Schoology To-Do</h1>
          <button onClick={handleSignOut} className="text-sm text-gray-500 hover:text-gray-700 underline">Sign out</button>
        </div>
      </header>
      ...
      <div className="flex-1 px-4 sm:px-8 py-6">
        <div className="max-w-7xl mx-auto">
          ...
          {(needsSetup || showUrlEditor) && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 mb-6 max-w-2xl">
              ...
              <div className="flex gap-3">
                ...
                {showUrlEditor && (
                  <button onClick={() => setShowUrlEditor(false)} className="text-sm text-gray-500 hover:text-gray-700 px-3">Cancel</button>
                )}
              </div>
          ...
          {initializing ? (
            ...
            <p className="text-sm text-gray-400 text-center py-24">No assignments yet. Click Sync to import from Schoology.</p>
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          ...
                          <tbody className="divide-y divide-gray-100">
                            {todoList.map((a) => (
                              <tr key={a.id} className={`group transition-colors ${a.status ? 'bg-gray-50/60' : 'hover:bg-gray-50'}`}>
                                ...
                                <td className={`px-4 py-3 ${a.status ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                <div>
                  ...
                  <div className="space-y-4">
                            ...
                            <div key={a.id} className="px-4 py-3">
                              <div className="flex items-start justify-between gap-2">
                                <div className="min-w-0 flex-1">
                                  <p className="text-sm font-medium truncate text-gray-900">{a.title}</p>
                    ...
                    {upcomingGroups.length === 0 && (
                      <p className="text-sm text-gray-400 text-center py-8">No upcoming assignments.</p>
                    )}
                  </div>
                </div>
              </div>
      ...
      {savedIcalUrl && !showUrlEditor && (
        <footer className="bg-white border-t border-gray-200 px-4 sm:px-8 py-3">
          <div className="max-w-7xl mx-auto flex items-center justify-between text-sm text-gray-500">
            <span>
              {lastSyncedAt ? `Last synced: ${timeAgo(lastSyncedAt)}` : 'Not synced yet'}
              {syncMessage && <span className="ml-3 text-green-600 font-medium">{syncMessage}</span>}
              {syncError && <span className="ml-3 text-red-600">{syncError}</span>}
            </span>
...
function ThSortable({ label, sortKey: key, active, dir, onSort }: {
  ...
  return (
    <th className="px-4 py-3">
      <button onClick={() => onSort(key)} className="flex items-center gap-0.5 hover:text-gray-700 transition-colors">
        {label}
